...
def cal_uncertainty(attn, dropout, exp_rate, num_sample=20):
    with torch.no_grad():
        sample_list = [dropout(attn).requires_grad_(False) for _ in range(num_sample)]
...
class U_Adapter(nn.Module):
    def __init__(self, adapter_dim=8, theta=0.5, use_cdc=True, dropout_rate=0.2, hidden_dim=768):
        ...
        if use_cdc:
            self.adapter_conv = Conv2d_cd(in_channels=adapter_dim, out_channels=adapter_dim, kernel_size=3, stride=1, padding=1, theta=theta)
        else:
            self.adapter_conv = nn.Conv2d(in_channels=adapter_dim, out_channels=adapter_dim, kernel_size=3, stride=1, padding=1)
        self.cross_attention = MHA_UC(dim=adapter_dim)
        self.ln_before = nn.LayerNorm(adapter_dim)
        self.adapter_down_1 = nn.Linear(hidden_dim, adapter_dim)
        self.adapter_down_2 = nn.Linear(hidden_dim, adapter_dim)
    ...
    def forward(self, x, x_c, x_u):
        B, N, C = x.shape
        x_down_1 = self.adapter_down_1(x)
        x_down_1 = self.act(x_down_1)
        x_down_2 = self.adapter_down_2(x_c)
...
class MMDG(nn.Module):
    def __init__(self, dropout_ratio=0.3, exp_rate=2.25, num_sample=20, adapter_dim=8, r_ssp=0.3):
        ...
        self.fc_all = nn.Sequential(nn.Linear(dim + dim + dim, 2))
        self.adapter_1_2_2 = nn.Sequential([U_Adapter(adapter_dim=adapter_dim) for _ in range(self.num_encoders)])
        self.adapter_2_1_2 = nn.Sequential([U_Adapter(adapter_dim=adapter_dim) for _ in range(self.num_encoders)])
        self.adapter_1_3_2 = nn.Sequential([U_Adapter(adapter_dim=adapter_dim) for _ in range(self.num_encoders)])
        self.adapter_3_1_2 = nn.Sequential([U_Adapter(adapter_dim=adapter_dim) for _ in range(self.num_encoders)])
        self.mc_drop_list_1 = nn.ModuleList([MCDropout(p=dropout_ratio) for _ in range(self.num_encoders)])
        self.mc_drop_list_2 = nn.ModuleList([MCDropout(p=dropout_ratio) for _ in range(self.num_encoders)])
        self.mc_drop_list_3 = nn.ModuleList([MCDropout(p=dropout_ratio) for _ in range(self.num_encoders)])
    ...
    def forward(self, x1, x2, x3, domain=None):
        b, c, fh, fw = x1.shape
        x1 = self.conv_proj1(x1).flatten(2).transpose(1, 2)
        x2 = self.conv_proj2(x2).flatten(2).transpose(1, 2)
        x3 = self.conv_proj3(x3).flatten(2).transpose(1, 2)
        x1 = torch.cat((self.class_token1.expand(b, -1, -1), x1), dim=1)
